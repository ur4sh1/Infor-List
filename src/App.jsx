    ...
    <Flex h="100vh" align="center" justify="center" fontSize="20px" fontFamily="poppins">
      <Box maxW={900} w="100%" h="100vh" py={10} px={2}>
        ...
        <Table mt="6">
          <Thead>
            <Tr>
              <Th maxW={isMobile ? 5 :180} fontSize="20px">Nome</Th>
              <Th maxW={isMobile ? 5 :180} fontSize="20px">E-mail</Th>
              <Th maxW={isMobile ? 5 :120} fontSize="20px">Telefone</Th>
              <Th p={0}></Th>
              <Th p={0}></Th>
            </Tr>
          </Thead>
          <Tbody>
            {data.map(({name, email, tel}, index)=>(
              <Tr key={index} cursor="pointer" _hover={{bg: "gray.100"}}>
                <Td maxW={isMobile ? 5 :180} fontSize="20px">{name}</Td>
                <Td maxW={isMobile ? 5 :180} fontSize="20px">{email}</Td>
                <Td maxW={isMobile ? 5 :120} fontSize="20px">{tel}</Td>
                <Td p={0}>
                  <EditIcon fontSize={20} 
                  onClick={()=>[
                    setDataEdit({name, email, tel, index}),
                    onOpen(),
                  ]}/>
                </Td>
                <Td>
